...
function isDisabled(d: Date): boolean {
  if (props.minDate && d < new Date(props.minDate.getFullYear(), props.minDate.getMonth(), props.minDate.getDate())) return true
  if (props.maxDate && d > new Date(props.maxDate.getFullYear(), props.maxDate.getMonth(), props.maxDate.getDate())) return true
  return false
}
      ...
      <div class="pc-hdr-center">
        <div class="pc-selector-wrapper" @click.stop>
          <button
            ...
            @click.stop="showMonthPicker = !showMonthPicker; showYearPicker = false"
          >
            {{ monthLabel }}
            <ChevronDown :size="12" />
          </button>
        ...
        <div class="pc-selector-wrapper" @click.stop>
          <button
            ...
            @click.stop="showYearPicker = !showYearPicker; showMonthPicker = false"
          >
            {{ viewYear }}
            <ChevronDown :size="12" />
          </button>
...
.pc-nav:hover { background: var(--bg-hover); color: var(--text-primary); }
.pc-hdr-center {
...
.pc-selector:hover { background: var(--bg-hover); }
.pc-selector--active { background: var(--bg-active); }
.pc-dropdown {
...
.pc-dropdown--month { min-width: 80px; }
.pc-dropdown--year { min-width: 70px; }
.pc-dropdown-item {
...
.pc-day {
  ...
  height: 34px; border-radius: var(--radius-sm); border: none;
  background: transparent; font-size: 13px; font-weight: 400;
  color: var(--text-primary); cursor: pointer; transition: all 0.12s ease;
}
.pc-day:hover:not(.pc-day--off):not(.pc-day--sel) { background: var(--bg-hover); }
.pc-day--today:not(.pc-day--sel) { font-weight: 600; color: var(--accent); }
.pc-day--today:not(.pc-day--sel)::after {
  content: ''; position: absolute; inset: 4px;
  border-radius: var(--radius-xs); border: 1.5px solid var(--accent);
  pointer-events: none;
}
.pc-day--sel {
...
.pc-day--out { color: var(--text-tertiary); opacity: 0.4; }
.pc-day--out:hover:not(.pc-day--off) { opacity: 0.65; }
.pc-day--off { color: var(--text-tertiary); opacity: 0.25; cursor: not-allowed; }
@keyframes fadeIn {
