...
<template>
  <div class="settings-group">
    ...
    <div class="sg-row">
      <div class="sg-label"><div class="sg-hint">{{ t('sg_tpl_vars_h') }}</div></div>
    </div>
    ...
    <div v-for="v in tplVarList" :key="v.name" class="tpl-var-block">
      <div class="sg-row">
        ...
        <Button size="sm" variant="outline" class="min-w-[100px]" @click="removeVar(v.name)">{{ t('tpl_vars_delete') }}</Button>
      </div>
...
.sg-row--clickable:hover { background: var(--bg-hover); }
...
.sg-arrow { width: 16px; height: 16px; color: var(--text-tertiary); flex-shrink: 0; transition: transform 0.2s; }
.sg-arrow--rotated { transform: rotate(180deg); }
.sg-input--block { width: 100%; padding-left: 16px !important; }
.pwd-change-form { margin: 4px 0 8px; padding: 20px 24px; background: var(--bg-hover); border-radius: var(--radius-md); border: 1px solid var(--border-subtle); }
