...
import {
  Copy, Image as ImageIcon, Link, ExternalLink, FileText, Folder,
  Star, Archive, ArchiveRestore, Trash2, Lock, Clock, MoreHorizontal,
  ...
  >
      ...
      <div class="cell-content-inner">
        ...
        <template v-if="!display.isItemVisible(item)">
          <div class="cell-protected-mask">
            ...
            <Button variant="outline" size="sm" class="h-7 px-3 text-[11px] rounded-md" @click.stop="emit('open-protection', item)">{{ t('item_unlock') }}</Button>
          </div>
        </template>
        <!-- 图片预览 -->
        ...
        <!-- URL 链接样式 -->
        <span v-else-if="item.type === 'link' || detectContentType(display.displayContent(item)) === 'url'" class="cell-link-preview">
    ...
    <TableCell>
      <div class="cell-actions">
        ...
        <Button variant="ghost" size="icon-sm" class="btn-action-hide" @click="emit('preview', item)" :title="t('preview')">
        ...
        <template v-if="isArchive">
          <Button variant="ghost" size="icon-sm" class="btn-action-hide" @click="emit('unarchive', item)" :title="t('unarchive_action')">
            <ArchiveRestore :size="14" />
          </Button>
          <Button variant="ghost" size="icon-sm" class="btn-action-hide danger" @click="emit('delete', item)" :title="t('delete')">
            <Trash2 :size="14" />
          </Button>
        </template>
        ...
        <!-- 主列表视图：复制 / 收藏 / 删除 / 更多 -->
        <template v-else>
          <Button v-if="item.type !== 'file' || display.hasLocalPath(item)" variant="ghost" size="icon-sm" class="btn-action-hide" @click="emit('copy', item)" :title="t('copy')">
            <Copy :size="14" />
          </Button>
          <!-- 常驻：收藏（含收藏夹 popover） -->
          ...
          <div class="more-wrap">
            <Button variant="ghost" size="icon-sm" class="btn-action-hide" :title="t('more_actions')" @click.stop="emit('toggle-more', item)">
            ...
            <div v-if="moreOpenId === item.id" class="more-dropdown" @click.stop>
              ...
              <button v-if="item.type === 'file' && display.hasLocalPath(item)" type="button" class="more-item" @click="emit('reveal', item)">
              ...
              <div class="more-sep" />
              <button type="button" class="more-item more-item--accent" @click="emit('expiry-from-dropdown', item, $event)">
