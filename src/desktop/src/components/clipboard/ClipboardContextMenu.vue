...
import {
  ...
  Star, Archive, Trash2, Lock, Clock,
...
watch(() => props.item, (it) => { if (it) mode.value = props.initialMode ?? 'main' })

function ctxSetExpiryMode() { mode.value = 'expiry' }
function ctxBack() { mode.value = 'main' }
...
function actCopy() { if (props.item) { emit('copy', props.item); emit('close') } }
function actShare() { if (props.item) { emit('share', props.item); emit('close') } }
function actPreview() { if (props.item) { emit('preview', props.item); emit('close') } }
function actReveal() { if (props.item) { emit('reveal', props.item); emit('close') } }
function actProtection() { if (props.item) { emit('open-protection', props.item); emit('close') } }
function actDelete() { if (props.item) { emit('delete', props.item); emit('close') } }
  ...
  <template v-if="item">
    ...
    <div class="ctx-menu" :style="ctxStyle">
      <template v-if="mode === 'main'">
        <button type="button" class="ctx-item" @click="actCopy">
          <Copy :size="14" />{{ t('copy') }}
        </button>
        ...
        <button v-if="!isArchive && item!.type === 'file' && display.hasLocalPath(item!)" type="button" class="ctx-item" @click="actReveal">
        ...
        <button v-if="!isArchive" type="button" class="ctx-item" @click="actFavorite">
          <Star :size="14" :fill="item!.isFavorite ? 'currentColor' : 'none'" />{{ item!.isFavorite ? t('unfavorite') : t('favorite') }}
        </button>
