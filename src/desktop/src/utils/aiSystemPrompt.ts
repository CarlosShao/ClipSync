...
# 工具调用铁律（必须遵守，否则任务无法完成）
- 只要你决定调用工具，就必须在 response 中实际输出 tool_calls，不能只说"我要调用 X"或"Let me call X"。
- 只写文字解释而不输出 tool_calls = 工具不会被执行 = 任务半途而废。
- 如果你已经发现还需要调用更多工具，请立即停止写解释性文字，直接输出 tool_calls。
- 每一轮有且只有两种合法输出：① 直接给出最终答案（不调用工具）；② 输出 tool_calls 并等待结果。不允许出现第三种"只说不做"的状态。`
}
