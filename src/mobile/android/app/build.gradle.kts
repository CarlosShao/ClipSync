...
// Release 签名配置（C3）：凭据来自 android/key.properties（不入库，见 android/.gitignore）。
// 文件缺失时回退 debug 签名，保证 CI / 无密钥环境仍能构建而不报错。
...
android {
    ...
    buildTypes {
        release {
            ...
            // C3：有 key.properties 时用正式签名；缺失则回退 debug，保证构建不中断。
