...
const FEATURES = {
  ...
  encryption_model: {
    ...
    details:
      ...
      '图片/文件类：content_encrypted 存的是服务端磁盘文件名（uploads/images、uploads/files），字节未加密落盘，AI 拥有读取权限（仅图片像素需视觉模型识别）。',
  },
}
...
export function getPrivacyModelDoc() {
  return [
    ...
    '这两类 AI 都无法读取其字节——遇到时它会明确告诉你「这是本地/路径引用条目，请在应用内或本机查看」，并会尽量给出可定位的路径信息。',
    '',
    '**3. 高级密码保护（advanced）**',
