...
const FEATURES = {
  ...
  encryption_model: {
    ...
    details:
      ...
      '服务端可解密用于同步；本地临时条目（local-/text-/img-/file-/browser- 前缀）是桌面端捕获后、服务端确认写入前的乐观项，' +
      '此时只存在于前端内存，未入服务端库，AI 自然查不到。' +
      '高级密码保护条目：用独立 DEK 加密，必须用户密码/恢复密钥才能还原。' +
      '图片/文件类：content_encrypted 可能是内联 base64 data URL，也可能是服务端磁盘文件名（uploads/images、uploads/files），字节完整在服务端，AI 拥有读取权限（仅图片像素需视觉模型识别）。',
  },
}
...
export function getPrivacyModelDoc() {
  return [
    ...
    '',
    '**1. 服务端静态加密（文本/代码/链接类）**',
    '文本、代码、链接类条目以 `content_encrypted` 存储，使用服务端主密钥（环境变量 `ENCRYPTION_KEY`，AES-256-GCM）加密。',
    '服务端本身持有主密钥，因此可为同步/AI 读取等用途解密。这属于「传输与存储加密」，不是「只有用户本人能解」。',
    '',
    '**1b. 图片 / 文件类：字节就在服务端（两种形态）**',
    '形态 A（内联）：桌面端常把图片/文件以 base64 `data URL` 直接写进 `content_encrypted` 字段，字节完整存在服务端数据库。',
    '形态 B（磁盘）：走媒体上传接口时，字节落盘在 `uploads/images`、`uploads/files`（分片上传在 `uploads/` 根），`content_encrypted` 存的是磁盘文件名。两种形态在服务端都**未加密存储（local 模式）**。',
    '也就是说：前端能预览，是因为服务端持有完整字节（读盘或读库返回）；AI 大管家同样拥有对这些字节的读取与检索权限——不存在「非原文所以读不了」的情况。',
    '唯一真实边界：图片是二进制像素，纯文本模型无法「看见」内容，需接入支持视觉（vision）的模型才能识别；但这属于模型能力边界，不是权限边界。',
    '',
    '**2. 未同步的本地临时项 / 路径引用（AI 读不到字节）**',
    '桌面端刚捕获一条内容时，会先在列表里插入一个乐观项，使用 `local-` / `text-` / `img-` / `file-` / `browser-` 这类临时 ID。',
    '这些临时项**在服务端确认写入之前不会入库**，只存在于你的设备内存/前端状态里；等同步完成后，ID 会被替换为标准 UUID。因此 AI 后端此时查不到它们——这不是权限问题，是数据还没传到服务端。',
    '此外，「文件复制」条目（`content_type=file` 且 content_encrypted 是源文件路径）只在服务端保留路径，**文件字节未上传**，仍存在于你的设备上。',
    '这两类 AI 都无法读取其字节——遇到时它会明确告诉你「这是未同步的本地临时项」或「这是路径引用条目」，并会尽量给出可定位的路径信息。',
    '',
    '**3. 高级密码保护（advanced）**',
    ...
    '- 能（图片/文件）：读取其完整元数据；对文本/代码类文件，read_clip_content 会直接读盘返回原文内容。字节存储于服务端磁盘，AI 拥有读取权限。',
