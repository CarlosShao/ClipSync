...
router.put('/', apiLimiter, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO ai_settings (user_id, default_provider_id, default_model, selected_models, default_mode, thinking_enabled, thinking_strength, memory_enabled, parallel_enabled, created_at, updated_at)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING *`,
      [req.userId, s.defaultProviderId, s.defaultModel, JSON.stringify(s.selectedModels), s.defaultMode, s.thinkingEnabled, s.thinkingStrength, s.memoryEnabled, false]
    )
