...
// 幂等中间件（requireHeader：仅当客户端携带 Idempotency-Key 时才生效）
const idempotencyMiddleware = createIdempotencyMiddleware({ requireHeader: true });

// ============================================================
// Rust 桌面端直传接口（绕过 Tauri 事件系统，直接 HTTP POST）
// ============================================================
// 认证方式：X-Device-ID header（设备 ID 关联到 user_id）
// 用途：Rust clipboard monitor 检测到剪贴板变化后，直接 POST 到服务器，
// 服务器通过 WebSocket 广播给该用户的所有在线设备。
// 前端不再依赖 Tauri 事件总线接收剪贴板更新。
router.post('/direct', async (req, res) => {
  try {
    ...
    const deviceResult = await pool.query(
      'SELECT id, device_name, user_id FROM devices WHERE id = $1',
      [sourceDeviceId]
    );
    ...
    // 检测内容类型
    const detectedType = contentType && ['text', 'image', 'file', 'link', 'code'].includes(contentType)
      ? contentType : 'text';
    ...
    const result = await pool.query(
      ...
      [userId, sourceDeviceId, detectedType, contentEncrypted, cleanPreview, contentSize || 0, JSON.stringify(metadata || {}), contentHash, expiresAt || null]
    );
    ...
    // 审计日志
    await logAuditEvent(userId, AUDIT_ACTIONS.CLIPBOARD_CREATE, 'clipboard', item.id, {
      contentType: detectedType,
      sourceDeviceId,
      contentSize,
    }, { userId, deviceId: sourceDeviceId });
    ...
    logger.error('Direct clipboard sync error:', { error: err.message });
