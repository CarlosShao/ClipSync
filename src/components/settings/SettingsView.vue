...
const emit = defineEmits<{
  (e: 'open-modal', type: string): void
}>()

const { t } = useI18n()
const configStore = useConfigStore()
const { setMode } = useTheme()
const themeMode = currentMode
</script>
  ...
  <div class="settings-view">
    ...
    <div class="settings-group">
      ...
      <div class="sg-row">
        ...
        <button class="toggle on" @click="$event.currentTarget.classList.toggle('on')" />
      </div>
      <div class="sg-row">
        <div class="sg-label"><div class="sg-name">{{ t('sg_imgcomp') }}</div><div class="sg-hint">{{ t('sg_imgcomp_h') }}</div></div>
        <button class="toggle" @click="$event.currentTarget.classList.toggle('on')" />
      </div>
      <div class="sg-row">
        <div class="sg-label"><div class="sg-name">{{ t('sg_interval') }}</div><div class="sg-hint">{{ t('sg_interval_h') }}</div></div>
        <select class="styled-select">
          <option value="0">{{ t('int_rt') }}</option>
          <option value="5">{{ t('int_5m') }}</option>
          <option value="15">{{ t('int_15m') }}</option>
        </select>
      </div>
    </div>
    ...
    <div class="settings-group">
      ...
      <div class="sg-row" style="cursor:pointer;" @click="emit('open-modal', 'themes')">
        ...
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="sg-arrow">
      ...
      <div class="sg-row">
        ...
        <div class="mode-seg">
          <button :class="['mode-btn', { active: themeMode === 'light' }]" @click="setMode('light')">{{ t('mode_light') }}</button>
          <button :class="['mode-btn', { active: themeMode === 'dark' }]" @click="setMode('dark')">{{ t('mode_dark') }}</button>
        </div>
      </div>
      <div class="sg-row">
...
.sg-control { flex-shrink: 0; font-size: 13px; color: var(--text-secondary); }
