...
const qpFilteredItems = computed(() => {
  let result = clip.items
...
async function qpSelectItem(item: ClipItem) {
  await clip.copyItem(item)
  emit('close')
}

function handleQpKeydown(e: KeyboardEvent) {
  const list = qpFilteredItems.value
  ...
  else if (e.key === 'Enter' && list[qpSelectedIndex.value]) { qpSelectItem(list[qpSelectedIndex.value]) }
...
<template>
  <div v-if="open" class="qp-overlay" @click.self="emit('close')">
    <div class="qp-panel" @keydown="handleQpKeydown">
      <div class="qp-search">
        ...
        <input v-model="qpSearch" type="text" :placeholder="t('search_ph')" autofocus class="qp-search-input" />
        <kbd class="qp-kbd">ESC</kbd>
      </div>
      <div class="qp-list">
        <div
          v-for="(item, idx) in qpFilteredItems"
          :key="item.id"
          :class="['qp-item', { sel: idx === qpSelectedIndex }]"
          @click="qpSelectItem(item)"
        ...
        <div v-if="qpFilteredItems.length === 0" class="qp-empty">
          <span class="text-tertiary" style="font-size:13px;">{{ t('empty_title') }}</span>
        </div>
      </div>
      <div class="qp-footer">
...
.qp-item { display: flex; align-items: center; gap: 10px; padding: 8px 16px; cursor: pointer; transition: background 0.08s; }
