...
> **文档版本**: v2.1
> **适用版本**: ClipSync 0.1.0+
> **最后更新**: 2026年6月23日 20:13
> **文档 owner**: 产品测试专员
> **目标**: 将产品测试至可商用部署的质量标准
>
> **v2.2 更新说明**：统一Docker容器命名规范，与word-helper-mobile项目保持一致。项目名：`clipsync`，容器名：`clipsync`（API）、`clipsync-db`（PostgreSQL）、`clipsync-redis`（Redis）。
>
> **v2.0 更新说明**：后端服务启动方式从直接 Node.js/PM2 启动全面迁移为 Docker Compose 容器化部署，新增开发/生产环境分离配置。
