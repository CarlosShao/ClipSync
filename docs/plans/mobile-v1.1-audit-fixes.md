...
| C1 收藏夹管理 | ✅ 缺口已闭合（多选移动补齐） | hy4+X1 | 8df81a7+本次 |
| C2 搜索增强 | ✅ 缺口已闭合（单条删除含后端新端点；自定义 dateTo 起止选择器） | hy4+X2 | 41d7933+本次 |
| C3 条目管理 | ✅ 缺口已闭合（标签 chips 展示；服务端列表 expires_at 过滤+移动端假空态保护） | hy4+X1/X2 | 6b5eb54+本次 |
| C4 模板增强 | ✅（轻微：未封装变量 DELETE /:name） | hy4/复核 ZCode | c4a09c0 |
| C5 共享链接+通知中心 | ✅ | hy4/复核 ZCode | 36ebc61 |
| C6 账号资料 | ✅ 缺口已闭合（套餐徽标） | hy4+X2 | 9cfd95d+本次 |
