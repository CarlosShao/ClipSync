...
2. 提交：**营业执照、经营者身份证、经营者本人银行卡（结算用）、经营场景照片**
   ...
   - ⚠️ 原写「必须有公众号/小程序 AppID」——**仍需 AppID，但来源更灵活**：
     Native（扫码）下单接口的 `appid` 是**必填**参数（官方文档 `4012791877`），
     且**移动应用 / 小程序 / 公众号三种 AppID 任选其一**，只要与 mchid 绑定即可。
     但申请 **Native 支付权限**时，PC 网站场景需填「PC 网站对应的服务号或公众号
     APPID」并上传**网站授权函**（官方文档 `4012791875`）。
     → 结论：**仍要注册一个公众号/服务号（或小程序），这是绕不开的一步。**
