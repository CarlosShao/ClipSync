...
| PD4 | ~~用户侧无退款入口~~ **2026-09-19 推翻**：个人资料页「申请退款」；**2026-09-20 再改：申请≠退款，审核通过才动钱** | 支付宝退款是**同步且不可撤销**的 ⇒ 审核只能前置在调用渠道之前。用户点申请 = `POST /refund-request`：只落 `refund_requests`(pending) + **立即收回权益**，零渠道调用；管理台点「通过」才 `refundPaidOrder` 真打款，点「驳回」按申请时存的快照还原权益（用户若已另购新套餐则不还原，防再造双 active）。入口闸（`refundPolicy.js` 唯一实现，与列表同源）：①必须是**当前生效订阅（active 且未到期）的最近一笔已支付订单**（锚点，退掉即订阅 canceled ⇒ 锚点消失 ⇒ 历史单永不顺移可退）②支付后 ≤ `refund_self_window_days` 天（**后台可配**，默认 7）③渠道仅 alipay。超期单**仍显示入口**（点了给中文提示「已超过 N 天…请联系客服」），藏起来用户只会以为功能坏了 | 客户端 PlanManagementCard「申请退款」+「退款审核中，预计 N 个工作日原路退回」；管理台 `/refund-review`（通过/驳回 + 时限配置）；`GET /api/payments/refundable-orders` 每单带 `refundRequest` |
...
| F5b | 申请被入口闸拒 | 用户点「申请退款」里非锚点的单 / 超窗的单 | 弹窗列表预标 reasonCode；超期单**仍给按钮**，点了只提示不发请求 | 非锚定单 → 409 `NOT_CURRENT_SUB_ORDER`；支付超 `refund_self_window_days` 天或 paid_at 缺失 → 409 `REFUND_WINDOW_EXPIRED`（fail-closed）；已有一条在途申请 → 409 `REFUND_REQUEST_PENDING`。一律零渠道调用 | 无 | **已实现，单测通过** | `refundPolicy.js`、`refundRequest.js`、`PlanManagementCard.vue` |
| F5c | 可退订单清单 | `GET /api/payments/refundable-orders`（本人最近 10 条 paid/refunded） | 退款弹窗数据源；升级单摊开 原价/折抵/实付；每单带 `refundRequest` | flag 关闭 503；不返回隐私字段；reasonCode 与申请端点同源；`windowDays`/`reviewBusinessDays` 由后台配置驱动（客户端不得写死） | 无 | **已实现，单测通过** | `payments.js`、`refundPolicy.js` |
| F5d | **顺移退历史单**（薅羊毛攻击） | 用户申请掉锚定单后，再点列表里上一笔已付单 | 列表显示「不属于当前订阅，请联系客服」 | 申请即把订阅 `canceled` → `findSelfRefundAnchorOrderId` 返回 null → 其余任何 paid 单一律 409，零渠道调用 | 无（旧口径在此处可被连续退款，2026-09-20 已堵） | **已实现，回归用例 + 生产实测通过**（测试号 4 单全 `refundable=false`） | `refundPolicy.js`、`tests/refund-request.test.js` |
| F5e | **审核阶段本身**（申请≠退款） | 用户提交申请后等待处理 | 卡片/弹窗显示「退款审核中，预计 N 个工作日原路退回」；权益已停 | 管理台 `/refund-review`：通过 = CAS `pending→processing` 再 `refundPaidOrder` 真打款 → `approved`；渠道失败退回 `pending` 可重试；驳回必填理由 → 按快照还原权益（用户已另购新套餐则不还原）。重复审核拿不到 pending → 409，不会退两次钱 | 通过=全额原路退回；驳回=分文不动 | **已实现，单测通过（含重复审核/渠道失败/驳回还原三类）**；生产全链路待复验 | `refundRequest.js`、`admin/refundReviews.js`、`admin/refundSettings.js` |
| F5f | **客户端何时知道档位变了**（审核结果回灌 UI） | 管理台在应用外侧驳回/通过退款后，用户回到桌面端 | 侧栏「升级」小胶囊、当前档位、限额一起反映服务端真相，不停在旧档 | 客户端原本只在①启动/登录 ②自己按下付款/申请退款 两处重拉 `auth/me` 冗余字段；档位快照（`/subscriptions/current`）与 `auth/me` 是两个源，进个人资料只更新前者 → 出现「卡片已还原 Pro、侧栏还在催升级」。现修：窗口重获焦点即 `invalidatePlanLimits` + `invalidateCurrentSubscription` + `fetchUserProfile` + 广播（60s 节流），卡片挂载时也顺手重拉 `auth/me` | 服务端 WS 只有 `register/clipboard/ping`，**没有按用户推送通道**：用户不切窗口/不重启，最坏可一直看到旧档 | **已实现（c006dd3），焦点触发路径待真机确认**（打包版走 Tauri `onFocusChanged`，webview 里 DOM focus 不保证） | `HomeView.vue`、`PlanManagementCard.vue` |
